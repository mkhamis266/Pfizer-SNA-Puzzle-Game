    ...
    $(elem).droppable({
      drop: function (event, ui) {
        ...
        if (isSorted(currentList)) {
          p2_scour = ((1 / (imagePuzzle.stepCount * timerFunction)) * 100000).toFixed(0);
          localStorage.setItem("p2_scour", p2_scour);
          $(".score").empty();
          $(".score").text(p2_scour);
...
function isSorted(arr) {
  ...
  const p2_scour = ((1 / (imagePuzzle.stepCount * timerFunction)) * 100000).toFixed(0);
  localStorage.setItem("p2_scour", p2_scour);
  return true;
}
